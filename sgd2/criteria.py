...
def ideal_edge_length(pos, G, k2i, device, targetLengths=None, sampleSize=None, sample=None, reduce='mean'):
    ...
    if sample is not None:
        ...
        sampleSize = min(sampleSize, G.number_of_edges())
    ...
    targetLengths = torch.tensor([targetLengths[e] for e in edges], device=device)
...
def stress(pos, D, W, device, sampleSize=None, sample=None, reduce='mean'):
    if sample is None:
        n,m = pos.shape[0], pos.shape[1]
...
def edge_orthogonality(pos, G, k2i, sample=None, reduce='mean'):
    # 将索引值转为坐标 (S, 2)
    source = pos[sample[0], :]
    target = pos[sample[1], :]
    edge_vecs = F.normalize(source - target, dim=1)
    # l = torch.abs(edge_vecs[:, 0] * edge_vecs[:, 1])
    l = torch.abs(edge_vecs[:, 0] * edge_vecs[:, 1] * (edge_vecs[:, 0] - edge_vecs[:, 1]))
