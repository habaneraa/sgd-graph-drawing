...
def stress(pos, D, W, device, sampleSize=None):
    return C.stress(pos, D, W, device, sampleSize, reduce='mean').item()


def ideal_edge_length(pos, G, k2i, device, targetLengths=None, sampleSize=None):
    return C.ideal_edge_length(pos, G, k2i, device, targetLengths, sampleSize).item()
